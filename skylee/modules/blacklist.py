	...
	if len(words) > 1:
		...
		if len(to_unblacklist) == 1:
			if successful:
				...
				send_message(update.effective_message, "This is not a blacklist trigger!")
			...
			send_message(update.effective_message, "None of these triggers exist so it can't be removed.".format(
...
def blacklist_mode(update, context):
	...
	if conn:
		...
		if update.effective_message.chat.type == "private":
			send_message(update.effective_message, "This command can be only used in group not in PM")
			return ""
		chat = update.effective_chat
		chat_id = update.effective_chat.id
