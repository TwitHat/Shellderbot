...
def filters(update, context):
	...
	if not msg.reply_to_message and len(args) < 2:
		send_message(update.effective_message, "Please provide keyboard keyword for this filter to reply with!")
		return

	if msg.reply_to_message:
		if len(args) < 2:
			send_message(update.effective_message, "Please provide keyword for this filter to reply with!")
			return
		else:
			keyword = args[1]
	else:
	...
	if not msg.reply_to_message and len(extracted) >= 2:
		...
		send_message(update.effective_message, "Please provide keyword for this filter reply with!")
		return
			...
			if filt.reply == "there is should be a new reply":
				...
				try:
					...
					if excp.message == "Unsupported url protocol":
						try:
							send_message(update.effective_message, "You seem to be trying to use an unsupported url protocol. "
